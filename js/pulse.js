(function () {
  ...
  let apiAvailable = null; // null unknown, true live API, false static/offline
  let feedSource = "unknown"; // "api" | "static" | "none"
  ...
  async function attachImageFromBlob(file, label) {
    if (!file) return false;
    if (!String(file.type || "").startsWith("image/")) {
      setStatus("Please paste or choose an image file.", true);
      return false;
    }
    ...
    try {
      ...
      setStatus(label || "Image attached — ready to post.");
      return true;
    } catch (err) {
      setStatus(err.message || "Could not read image", true);
      clearPreview();
      return false;
    }
  }

  function setSetupNote(html, visible) {
    if (!els.setupNote) return;
    if (html != null) els.setupNote.innerHTML = html;
    els.setupNote.hidden = !visible;
  }

  function updateOfflineMessaging() {
    const googleOn = !!pulseConfig.google_enabled;
    if (apiAvailable === false) {
      setSetupNote(
        "Community feed is shown from the saved archive (read-only). " +
          "Posting, likes, and Google sign-in need the <strong>gallery server</strong> " +
          "(<code>start_server.bat</code> / Render). " +
          "Page-draw annotations are not wired yet — paste images into posts instead.",
        true
      );
      return;
    }
    if (apiAvailable === true && !googleOn) {
      setSetupNote(
        "Google Sign-In is optional. Without it, post with a display name. " +
          "To enable Google: copy <code>data/pulse-google-config.example.json</code> to " +
          "<code>data/pulse-google-config.json</code>, set your OAuth client ID, restart the server. " +
          "Page-draw annotations are not wired yet — paste or attach images to posts.",
        true
      );
      return;
    }
    if (apiAvailable === true && googleOn) {
      setSetupNote(
        "Paste or drop images into the compose box to attach them. " +
          "Page-draw annotations (draw on gallery pages) are not wired yet.",
        true
      );
      return;
    }
    // unknown
    if (els.setupNote) els.setupNote.hidden = true;
  }
  ...
  function updateAuthUI() {
    ...
    const canCompose = apiAvailable !== false && (!googleOn || signedIn);

    if (els.usernameField) els.usernameField.hidden = googleOn || apiAvailable === false;
    if (els.authSignedOut) els.authSignedOut.hidden = !googleOn || signedIn || apiAvailable === false;
    if (els.authSignedIn) els.authSignedIn.hidden = !signedIn;
    if (els.compose) els.compose.hidden = !canCompose;

    updateOfflineMessaging();
    ...
    if (googleOn && apiAvailable !== false && !googleReady) {
      initGoogleSignIn();
    }
  }
  ...
  function renderPost(post) {
    ...
    if (post.image_url) {
      imageHtml =
        '<div class="pulse-card-image">' +
        '<img src="' +
        escapeAttr(post.image_url) +
        '" alt="" loading="lazy" />' +
        '<button type="button" class="pulse-copy-image-btn" data-action="copy-image" title="Copy image">Copy image</button>' +
        "</div>";
    }
  ...
  async function loadStaticFeed() {
    const res = await fetch("data/pulse-posts.json", { cache: "no-store" });
    if (!res.ok) throw new Error("static feed missing");
    const data = await res.json();
    const posts = Array.isArray(data) ? data : data.posts || [];
    return posts;
  }

  function showFeedBanner(msg) {
    if (!els.feed || !msg) return;
    const banner = document.createElement("div");
    banner.className = "pulse-feed-banner";
    banner.innerHTML = "<p>" + msg + "</p>";
    els.feed.insertBefore(banner, els.feed.firstChild);
  }

  async function loadFeed() {
    if (els.feed) {
      els.feed.innerHTML = '<div class="pulse-empty"><p>Loading community feed…</p></div>';
    }
    try {
      const res = await fetch("/api/pulse/feed", { cache: "no-store" });
      ...
      /* fall through to static archive */
    }

    try {
      const posts = await loadStaticFeed();
      apiAvailable = false;
      feedSource = "static";
      renderFeed(posts);
      showFeedBanner(
        "Showing saved community feed (read-only). Live posting needs the gallery server."
      );
      updateAuthUI();
    } catch (_staticErr) {
      apiAvailable = false;
      ...
      if (els.feed) {
        els.feed.innerHTML =
          '<div class="pulse-empty">' +
          "<h3>Feed offline</h3>" +
          "<p>Could not reach the live Pulse API or the saved archive. " +
          "Start the gallery server (<code>start_server.bat</code>) or open the site from a host that serves <code>data/pulse-posts.json</code>.</p>" +
          "</div>";
      }
      updateAuthUI();
    }
  ...
  async function submitPost() {
    if (apiAvailable === false) {
      setStatus("Cannot post while offline — start the gallery server to send to the community feed.", true);
      return;
    }
  ...
  async function handleCardClick(ev) {
    ...
    if (action === "copy-image") {
      const img = card.querySelector(".pulse-card-image img");
      if (!img || !img.src) {
        setStatus("No image to copy.", true);
        return;
      }
      try {
        ...
        if (navigator.clipboard && navigator.clipboard.write && window.ClipboardItem) {
          ...
          setStatus("Image copied — paste into a new post or elsewhere.");
        } else {
        ...
        try {
          ...
          setStatus("Could not copy image (browser blocked clipboard).", true);
        }
      }
      return;
    }
    ...
    if (apiAvailable === false) {
      setStatus("Feed is read-only offline — start the gallery server to post or like.", true);
      return;
    }
  ...
  function initCompose() {
    ...
    if (els.file) {
      els.file.addEventListener("change", async function () {
        ...
        if (!file) return;
        await attachImageFromBlob(file, "Photo attached.");
      });
    }

    function onPasteImage(ev) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (item && item.type && item.type.indexOf("image") === 0) {
          ...
          if (file) {
            ev.preventDefault();
            attachImageFromBlob(file, "Pasted image attached.");
            return;
          }
        }
      }
    ...
    function onDropImage(ev) {
      ev.preventDefault();
      if (els.compose) els.compose.classList.remove("pulse-drag-over");
      const file =
        ev.dataTransfer &&
        ev.dataTransfer.files &&
        ev.dataTransfer.files[0];
      if (file) attachImageFromBlob(file, "Dropped image attached.");
    }
    ...
    function onDragOver(ev) {
      ...
      if (!hasFiles) return;
      ev.preventDefault();
      if (els.compose) els.compose.classList.add("pulse-drag-over");
    }
    ...
    if (els.text) {
      els.text.addEventListener("paste", onPasteImage);
    }
    if (els.compose) {
      els.compose.addEventListener("paste", onPasteImage);
      els.compose.addEventListener("dragover", onDragOver);
      els.compose.addEventListener("dragleave", onDragLeave);
      els.compose.addEventListener("drop", onDropImage);
    }
  ...
  loadConfig().then(function () {
    // Prefetch feed so first open is warm; tab show refreshes again
    if (document.getElementById("panel-pulse") && !document.getElementById("panel-pulse").hidden) {
      loadFeed();
    }
  });

  window.Pulse = { onShow: onShow, loadFeed: loadFeed };
